...
def plot(X, Y=None, xlabel=None, ylabel=None, legend=[], loc=None, title=None,
         ...
         alpha=1.0, grid=False, equal=False, figsize=(5,3), rasterized=False, axes=None):
    """
    Plot data points.
    ...
    fmts=fmts*((len(Y)-1)//len(fmts)+1) # extend the formats as needed
    for x, y, fmt in zip(X, Y, fmts):
        if len(x):
            ax.plot(x, y, fmt, linewidth=linewidth, markersize=markersize,
            	fillstyle=fillstyle,alpha=alpha,rasterized=rasterized)
        else:
            ax.plot(y, fmt, linewidth=linewidth, markersize=markersize,
            	fillstyle=fillstyle,alpha=alpha,rasterized=rasterized)
    ...
    return ax # useful if we started a new figure
...
def save_mib_img(imgname):
    """
    Saves the current plot to the img/ folder in my local directory
    """
    projectpath = '/Users/markusmeister/Work/Projects/Books/Mathematics Book/MIB Various'
    plt.savefig(projectpath+'/img/'+imgname, bbox_inches = "tight") 
